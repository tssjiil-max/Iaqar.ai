...
'''
anchor = 'import { buildOrchestratorEventId, dispatchOrchestratorEvent } from "./central-orchestrator-service.js";\n'
if completion_import not in s:
    if anchor not in s:
        raise SystemExit('orchestrator import anchor missing')
    s = s.replace(anchor, anchor + completion_import, 1)
...
elif 'syncOpportunityCoverage' not in s:
    raise SystemExit('coverage import anchor missing')
...
'''
handler_anchor = 'async function handleMatchingRun(request, env, requestId) {'
if 'async function handleCompletionSessionCreate' not in s:
    if handler_anchor not in s:
        raise SystemExit('matching handler anchor missing')
    s = s.replace(handler_anchor, handler_block + handler_anchor, 1)

route_anchor = '''      if (request.method === "POST" && url.pathname === "/pipeline/canonical-intake") {
        return await handleCanonicalIntakeStart(request, env, requestId);
      }'''
completion_routes = '''      if (request.method === "POST" && url.pathname === "/completion/sessions") {
...
'''
if '/completion/sessions' not in s:
    if route_anchor not in s:
        raise SystemExit('canonical intake route anchor missing')
    s = s.replace(route_anchor, completion_routes + route_anchor, 1)

if 'createPersistentCompletionSession' not in s or '/completion/sessions' not in s:
    raise SystemExit('completion runtime was not wired')
p.write_text(s)

p = Path('package.json')
s = p.read_text()
if '"test:completion:gate"' not in s:
    anchor = '    "test:admin:gate": "node --test test/admin-separation-phase11.test.mjs test/admin-console.test.mjs",\n'
    if anchor not in s:
        raise SystemExit('admin test script anchor missing')
    s = s.replace(anchor, anchor + '    "test:completion:gate": "node --test test/data-completion-runtime-phase12.test.mjs test/data-completion.test.mjs test/opportunity-readiness.test.mjs",\n', 1)
p.write_text(s)
