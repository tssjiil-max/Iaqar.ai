...
(function () {
  ...
  function buildNotificationTargetFromData(data = {}) {
    ...
    if (entityType === "opportunity" || entityId.startsWith("opp_")) {
      return { kind: "opportunity", id: entityId, officeId, focusFollowUp: Boolean(data.focusFollowUp) };
    }
  ...
  function buildNotificationRelativeUrl(data = {}) {
    ...
    switch (target.kind) {
      ...
      case "daily-task":
        if (target.id) {
          params.set("openDailyTask", target.id);
      ...
      case "opportunity":
        ...
        else if (!target.id) params.set("openNotifications", "1");
        break;
      case "cooperation":
