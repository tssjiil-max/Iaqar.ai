(() => {
    ...
    modalHost.innerHTML = `<div class="modal-overlay" role="presentation">
      <div class="modal" role="dialog" aria-modal="true" aria-labelledby="adminModalTitle">
        ...
        <div class="modal-actions">
          <button type="button" class="btn ${danger ? "danger" : "primary"}" id="adminModalConfirm">${escapeHtml(confirmLabel)}</button>
          <button type="button" class="btn secondary" id="adminModalCancel">${escapeHtml(cancelLabel)}</button>
        </div>
      </div>
    </div>`;
  ...
  function loginView(message = "") {
    destroyShell();
    root.innerHTML = `<div class="admin-workspace login-wrap">
      <section class="admin-card">
        <h2>دخول إدارة المنصة</h2>
        <p class="detail-row">هذا الدخول مخصص لمدير المنصة فقط.</p>
        <form id="adminLoginForm" class="form-stack" style="margin-top:10px">
          <input name="email" type="email" placeholder="البريد الإلكتروني" required autocomplete="username" aria-label="البريد الإلكتروني">
          <input name="password" type="password" placeholder="كلمة المرور" required autocomplete="current-password" aria-label="كلمة المرور">
        ...
        <p style="margin-top:12px;font-size:12px"><a href="/">العودة للمنصة العامة</a></p>
      </section>
    </div>`;
  ...
  function controlBarHtml(includeTabs = true) {
    return `${includeTabs ? officeTabs() : ""}
      <div class="control-bar">
        <input id="officeSearch" type="search" placeholder="بحث: اسم، وسيط، جوال، فال" value="${escapeHtml(state.filters.q)}" aria-label="بحث المكاتب">
        <select id="officeCity" aria-label="المدينة"><option value="">كل المدن</option>
          ${state.cities.map(c => `<option value="${escapeHtml(c)}" ${state.filters.city === c ? "selected" : ""}>${escapeHtml(c)}</option>`).join("")}
        </select>
        ${filterSelect("officeApprovalStatus", "حالة الاعتماد", state.filters.approvalStatus, [
        ...
        <select id="officeSort" aria-label="ترتيب النتائج">
        ...
        <button type="button" class="btn secondary" id="officeApplyFilters">تطبيق</button>
      </div>`;
  }
      ...
      setMainContent(`${pageHero("النشاط", PAGE_SUBTITLES.activity)}
        <section class="admin-card">
          ...
          <div class="control-bar">
            ...
            <select id="officeSort" aria-label="ترتيب">
              <option value="last_activity">آخر نشاط</option>
              <option value="most_active">الأكثر نشاطًا</option>
              <option value="least_active">الأقل نشاطًا</option>
            </select>
          ...
          <div id="activityData">${loadingHtml()}</div>
        </section>`);
    ...
    if (actions) {
      actions.innerHTML = officeActionButtons(o, false) +
        (o.approvalStatus === "approved" ? `
          ...
          <button type="button" class="btn secondary" data-note="${escapeHtml(o.officeId)}">إضافة ملاحظة</button>` : "");
      bindOfficeActions();
    }
  ...
  function deniedView() {
    ...
    root.innerHTML = `<div class="admin-workspace login-wrap">
      <section class="admin-card">
        <h2>الوصول مرفوض</h2>
        <p class="detail-row">هذا الحساب ليس من إدارة المنصة. لا يتم تحميل بيانات الإدارة.</p>
        <button type="button" class="btn secondary" id="adminLogout">تسجيل الخروج</button>
  ...
  async function render() {
    if (!window.firebase || !firebase.apps || !firebase.apps.length) {
      destroyShell();
      root.innerHTML = '<div class="admin-workspace"><section class="admin-card"><p>تعذر بدء المنصة. حدّث الصفحة.</p></section></div>';
      return;
    }
    const user = firebase.auth().currentUser;
    if (!user) {
