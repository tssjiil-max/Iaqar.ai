...
function partyControlButton(task, party, progress) {
  const isOwner = party === "owner"; const hasContext = Boolean(task.matchId && task.offerId && task.requestId); const disabled = task.dataIntegrity === "INVALID_TASK_DATA" || !hasContext;
  ...
  return `<button type="button" class="cv2-party-control${disabled ? " is-disabled" : ""}" ${attr}="${action}" data-party="${party}"${disabled ? " disabled" : ""}>${escapeContentHtml(disabled ? "تعذر ربط جلسة التفاوض" : label)}</button>`;
}
function partyProgressHtml(task = {}) { const client = partyProgress(task, "client"), owner = partyProgress(task, "owner"); const row = (label, progress) => `<div class="cv2-party-progress-row"><strong>${label}</strong><div>${progressStep("فتح واتساب", progress.openedWhatsApp)}${progressStep("فتح الرابط", progress.openedLink)}${progressStep("تم الرد", progress.replied)}</div></div>`; return `<div class="cv2-coop-block cv2-party-progress"><strong>مسار العميل والمالك</strong>${row("العميل", client)}${row("المالك", owner)}<p class="cv2-party-progress-note">فتح واتساب لا يعني أن الرابط وصل أو أن الطرف رد.</p><div class="cv2-party-controls">${partyControlButton(task, "client", client)}${partyControlButton(task, "owner", owner)}</div></div>`; }
...
function brokerNegotiationPanelHtml(task = {}) {
  const vm = buildNegotiationAssistant(task); if (!vm.interventionRequired) return "";
  const serious = task.seriousIntentConfirmed === true || String(task.viewingOutcome || "").toUpperCase() === "SERIOUS"; const context = `${task.livingStage || task.workflowStage || ""} ${task.coordinationOutcome || ""}`.toLowerCase();
  ...
  const options = context.includes("viewing") ? allOptions.slice(3) : context.includes("negotiation") || context.includes("price") ? allOptions.slice(1) : allOptions; const smartOptions = options.map((label) => `<option value="${escapeContentHtml(label)}">${escapeContentHtml(label)}</option>`).join("");
  return `<details class="cv2-broker-panel" data-broker-panel open><summary><span aria-hidden="true">♙</span> تدخل الوسيط</summary><div class="cv2-broker-panel-body"><p class="cv2-neg-intervention-line">${escapeContentHtml(vm.interventionLine)}</p><label>إجراء ذكي<select data-broker-preset>${smartOptions}</select></label><label>ملاحظة<textarea data-broker-note maxlength="1000" rows="2" placeholder="ملاحظة مختصرة"></textarea></label><label>إرسال إلى<select data-broker-audience><option value="both">الطرفين</option><option value="client">العميل</option><option value="owner">المالك</option></select></label><button type="button" class="cv2-exec-secondary" data-broker-action="note">حفظ وإرسال الملاحظة</button><div class="cv2-broker-decisions">${serious ? `<button type="button" class="cv2-exec-primary" data-broker-action="continue">متابعة وإتمام الصفقة</button>` : ""}<button type="button" class="cv2-exec-secondary" data-broker-action="no_agreement">لم يتم الاتفاق</button></div></div></details>`;
}
function matchGroupBodyHtml(task = {}) { const ranked = (task.candidates || []).map((item) => { const bits = [item.propertyLine, item.moneyLine, item.areaLine].filter(Boolean).join(" · "); return `<li>مرشح ${item.rank}: ${escapeContentHtml(bits || item.matchId || "")}</li>`; }).join(""); return `<div class="cv2-coop-expanded cv2-match-expanded">${negotiationAssistantHtml(task)}${matchHeroHtml(task)}${matchActionHtml(task)}${partyResponsesHtml(task)}${partyProgressHtml(task)}${brokerNegotiationPanelHtml(task)}${matchDetailsHtml(task)}${ranked && (task.candidates || []).length > 1 ? `<details class="cv2-match-fold"><summary><span>☷</span> المرشحون</summary><div class="cv2-match-fold-body"><ol>${ranked}</ol></div></details>` : ""}${timelineHtml(task)}</div>`; }
