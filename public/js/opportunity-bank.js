...
} from "./opportunity-action-projection-domain.js";
...
const state = {
  ...
  actionFilter: OPPORTUNITY_ACTION_FILTER.ALL,
  operations: [],
  unsubscribe: null,
  records: new Map(),
  facetMeta: [],
...
function actionIndex() {
  return buildOpportunityActionIndex(state.operations, { officeId: officeId(), now: new Date() });
}
...
function bankRowHtml(row) {
  ...
  return buildBankInboxCardHtml({ ...record, id: row.id }, {
    ...
    action: actionForOpportunity(row.id)
  });
}
...
function renderList() {
  ...
  const rows = [...records].sort((a, b) => compareOpportunityPriority(a, b, (record) => actions.get(record.id) || null)).map((record) => ({
...
function bindListClicks() {
  ...
  list.addEventListener("click", (event) => {
    ...
    const primaryAction = event.target.closest("[data-opportunity-primary-action]");
    if (primaryAction && list.contains(primaryAction)) {
      event.preventDefault();
...
function boot() {
  ...
  if (Array.isArray(window.IAQAR?.operationsItems)) {
    state.operations = window.IAQAR.operationsItems.filter((item) => String(item?.recordType || "").toLowerCase() === "operation");
  }
  ...
  $("bankActionFilters")?.addEventListener("click", (event) => {
    const button = event.target.closest?.("[data-bank-action-filter]");
    if (!button) return;
    state.actionFilter = button.getAttribute("data-bank-action-filter") || OPPORTUNITY_ACTION_FILTER.ALL;
    $("bankActionFilters")?.querySelectorAll("[data-bank-action-filter]").forEach((item) => {
