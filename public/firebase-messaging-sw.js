...
function buildNotificationRelativeUrl(data = {}) {
  ...
  else if (type === "match" && recordId && !recordId.startsWith("op_")) params.set("openMatch", recordId);
  else if (recordId) params.set("openOperation", recordId); else params.set("openNotifications", "1");
...
function notificationUrl(data = {}) {
  ...
  try { return new URL(relative, self.location.origin).href; } catch (_) { return new URL("/", self.location.origin).href; }
}

self.addEventListener("push", event => {
  const { notification, data } = notificationPayload(event);
  const title = notification.title || "IAQAR — مكاتب عقارية ذكية";
  const icon = notification.icon || data.iconUrl || "/icons/iaqar-notification.svg";
  ...
  const options = {
    ...
    icon,
    badge: notification.badge || "/icons/iaqar-badge.svg",
    dir: "rtl", lang: "ar",
    data: { url:absoluteLink, relativeUrl:relativeLink, officeId:data.officeId || "", type:data.type || "", recordId:data.recordId || data.matchId || data.dealId || "", matchId:data.matchId || "", dealId:data.dealId || "", deliveryId:data.deliveryId || "" },
  ...
  event.waitUntil(clients.matchAll({ type:"window", includeUncontrolled:true }).then(list => {
    ...
    if (visibleClients.length) { visibleClients.forEach(client => client.postMessage({ type:"IAQAR_FCM_FOREGROUND", payload:{ notification:{ title, body:options.body }, data:{ ...data, url:relativeLink } } })); return undefined; }
...
self.addEventListener("notificationclick", event => {
  ...
  event.waitUntil(clients.matchAll({ type:"window", includeUncontrolled:true }).then(list => {
    const sameOrigin = list.find(client => { try { return new URL(client.url).origin === self.location.origin; } catch (_) { return false; } });
    if (sameOrigin) return sameOrigin.navigate(targetUrl).then(client => client && "focus" in client ? client.focus() : client);
...
const IAQAR_SHELL = [
  "/", "/manifest.webmanifest", "/share-target.html",
  "/icons/iaqar-app.svg", "/icons/iaqar-notification.svg", "/icons/iaqar-badge.svg",
  "/icons/icon-192.png", "/icons/icon-512.png", "/icons/default-office.png",
  "/js/notification-navigation.js", "/js/access-gate.js", "/js/firebase-office.js", "/js/fcm-fid.js", "/js/office-settings.js", "/js/add-opportunity.js", "/js/opportunity-bank.js", "/js/qrcode.js", "/js/whatsapp-office.js", "/js/operations-domain-bridge.js", "/js/messaging-domain-bridge.js", "/js/workflow-office.js"
];
...
self.addEventListener("fetch", event => {
  ...
  event.respondWith(fetch(event.request).then(response => { if (response && response.ok) { const copy=response.clone(); caches.open(IAQAR_CACHE).then(cache => cache.put(event.request, copy)).catch(() => {}); } return response; }).catch(() => caches.match(event.request).then(cached => cached || caches.match("/"))));
});
