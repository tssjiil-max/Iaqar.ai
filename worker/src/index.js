...
function parsedToFirestoreFields(parsed, context) {
  const normalizedSource = normalizeOpportunitySource(context.source || "whatsapp");
  ...
  return compactFields({
    ...
    confidence: firestoreInteger(parsed.confidence || 0),
    completeness: firestoreInteger(parsed.completeness),
...
export {
  normalizeLoginPhone, legacyLocalLoginPhone, resolveLoginDirectory, isLoginDirectoryActive,
  firebaseServiceAccount, createServiceAccountJwt, buildNotificationLink, buildFcmTarget,
  buildFcmHttpMessage, parseFcmFailure,
  ...
  isArchivedLifecycle, isActiveLifecycle
};
