...
- Scripts load in a fixed order at the end of `<body>`: Firebase compat SDKs →
  `/__/firebase/init.js` → `runtime-config.js` (Phase 9A deploymentEnvironment +
  workerBase) → `fcm-fid.js` (module) → `access-gate.js` → `firebase-office.js` →
  `qrcode.js` → `office-settings.js` → `whatsapp-office.js` → inline operations
  script → `workflow-office.js`.
