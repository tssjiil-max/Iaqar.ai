...
| `MATCHING_REQUESTED` | **Partial** — runs automatically at the end of intake, and `workflow-office.js` `processNewPublicIntakes` auto-triggers the Worker for any intake still `status: "new"`. There is no rematch when an existing opportunity is edited. | `worker/src/index.js` `findAndSaveMatches`; `public/js/workflow-office.js` |
| `MATCH_CREATED` | **Real** — writes `offices/{id}/matches/{matchId}` plus a `timeline/evt_match_created` entry. Idempotent per pair through the deterministic hashed ID. Not versioned by matching-rule or data version. | `worker/src/index.js` `findAndSaveMatches` |
...
| Content-hash match ID `mat_{sha256(officeId|sortedPair)[0..36]}` + existence check | The same offer/request pair produces at most one match. | `worker/src/index.js` `findAndSaveMatches` |
